...
export default function CalendarComponent() {
  ...
  const [showEditClassModal, setShowEditClassModal] = useState(false); // State for edit class modal
  ...
  const handleSelectEvent = (event) => {
    ...
    setShowEditClassModal(true); // Show edit class modal when an event is clicked
  };
  ...
  const eventPropGetter = (event) => {
    const backgroundColor =
      event.status === "BOOKED_OFF" ? "#6b7280" : "#3b82f6"; // Grey for booked off, blue for others
    return {
      style: {
